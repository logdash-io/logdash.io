...
import { NotificationsChannelReadService } from '../../../notifications-channel/read/notifications-channel-read.service';
import { ClusterReadModule } from '../../read/cluster-read.module';
import { ProjectReadModule } from '../../../project/read/project-read.module';
import { NotificationsChannelReadModule } from '../../../notifications-channel/read/notifications-channel-read.module';
...
const NOTIFICATIONS_CHANNEL_ID_PARAM_NAME = 'notificationsChannelId';
...
export class ClusterMemberGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const notificationsChannelIdFromParams = request.params[NOTIFICATIONS_CHANNEL_ID_PARAM_NAME];
    ...
    if (
      !clusterIdFromParams &&
      !projectIdFromParams &&
      !notificationsChannelIdFromParams &&
    ...
    if (notificationsChannelIdFromParams) {
      return this.checkForNotificationsChannelId({
        notificationsChannelId: notificationsChannelIdFromParams,
        userId,
      });
    }
  ...
  private async checkForNotificationsChannelId(dto: {
    notificationsChannelId: string;
    userId: string;
  }): Promise<boolean> {
    const channel = await this.notificationsChannelReadService.readById(dto.notificationsChannelId);
