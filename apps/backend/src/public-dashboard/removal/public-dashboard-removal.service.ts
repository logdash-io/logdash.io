...
export class PublicDashboardRemovalService {
  ...
  public async deletePublicDashboardsByClusterId(clusterId: string): Promise<void> {
    ...
    for (const publicDashboard of publicDashboards) {
      await this.deletePublicDashboardById(publicDashboard.id);
    }
  }

  public async deletePublicDashboardById(publicDashboardId: string): Promise<void> {
