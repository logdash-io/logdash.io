<script lang="ts">
  ...
  function createDatasetForGranularity(
    ...
    return logMetrics[granularity]
      .filter(
        (metric) =>
          new Date(metric.date) >= timeWindow && new Date(metric.date) <= now,
      )
      .map((metric) => {
  ...
  function createChart(
    ...
    drawInteractionLayer(chart, data, xScale, yScale, innerWidth, innerHeight);
  ...
  function createYScale(
    ...
    const p90Value = values[Math.floor(values.length * 0.9)] || medianValue * 2;
  ...
  function drawBars(
    ...
    const barsGroup = chart.append('g').attr('clip-path', `url(#${clipId})`);
    ...
    data.forEach((point) => {
      ...
      LOG_TYPES.forEach((logType) => {
        if (point[logType] > 0) {
          ...
          const segmentValue = Math.min(point[logType], yMax - currentTotal);
  ...
  function showTooltip(event: any, data: ChartDataPoint) {
    const formattedDate = formatDateByGranularity(data.date, data.granularity);
    const colorScale = d3
      .scaleOrdinal<string>()
      .domain(LOG_TYPES)
      .range(LOG_COLORS);
  ...
  function renderEmptyState(
    ...
    d3.select(container)
      .append('div')
      .attr('class', 'flex items-center justify-center h-32 text-gray-400')
      .html(
  ...
  function renderAllCharts() {
    if (!logMetrics) {
      logger.warn('[LogMetricsChart] No log metrics data available');
      return;
    }
  ...
  $effect(() => {
    logger.debug('[LogMetricsChart] $effect renderAllCharts', {
      logMetrics,
      timeRanges,
    });
    ...
    <div class="space-y-10">
      ...
      <div class="chart-container">
        <div class="mb-2 flex justify-between">
          ...
          <span class="text-xs text-gray-400">
            Last {formatTimeRange(getTimeRangeForGranularity('minute'))}
          </span>
        </div>
        <div
      ...
      <div class="chart-container">
        <div class="mb-2 flex justify-between">
          ...
          <span class="text-xs text-gray-400">
            Last {formatTimeRange(getTimeRangeForGranularity('hour'))}
          </span>
        </div>
        <div class="chart-wrapper w-full" bind:this={hourChartContainer}></div>
      </div>
      ...
      <div class="chart-container">
        <div class="mb-2 flex justify-between">
          ...
          <span class="text-xs text-gray-400">
            Last {formatTimeRange(getTimeRangeForGranularity('day'))}
          </span>
        </div>
        <div class="chart-wrapper w-full" bind:this={dayChartContainer}></div>
      </div>
    </div>
  </div>
{:else}
