<script>
  import { page } from '$app/state';
  import NewsletterCheckbox from '$lib/domains/shared/ui/components/NewsletterCheckbox.svelte';
  import TosCheckbox from '$lib/domains/shared/ui/components/TOSCheckbox.svelte';
  import { generateGithubOAuthUrl } from '$lib/domains/shared/utils/generate-github-oauth-url';
  import { onMount } from 'svelte';
  import { cubicInOut } from 'svelte/easing';
  import { fade, scale } from 'svelte/transition';

  const needs_account = page.url.searchParams.get('needs_account');

  let termsAccepted = $state(false);
  let emailAccepted = $state(false);
  let loggingIn = $state(false);

  const handleGithubLogin = () => {
    loggingIn = true;
    window.location.href = generateGithubOAuthUrl({
      terms_accepted: termsAccepted,
      email_accepted: emailAccepted,
      flow: 'login',
      fallback_url: '/app/auth?needs_account=true',
      next_url: '/app/clusters',
    });
  };

  onMount(() => {
    return () => {
      loggingIn = false;
      termsAccepted = false;
      emailAccepted = false;
    };
  });
</script>

<div
  in:scale={{
    duration: 300,
    start: 1.1,
    easing: cubicInOut,
  }}
  out:scale={{
    duration: 300,
    start: 1.1,
    easing: cubicInOut,
  }}
  class="bg-base-300 flex min-h-screen w-full items-center justify-center"
>
  <div class="card ld-card-base w-96 rounded-2xl shadow-xl">
    <div class="card-body items-center p-6 text-center">
      <h2 class="card-title text-3xl font-bold">Logdash</h2>
      <p class="text-base-content/70 mb-4">
        {#if needs_account}
          You need to make an account to continue.
        {:else}
          You need to login to continue.
        {/if}
      </p>

      <div
        class={[
          'card-actions w-full justify-start gap-2 rounded-2xl',
          {
            'bg-base-200 border-primary/30 border p-4': needs_account,
            'p-2 pt-0': !needs_account,
          },
        ]}
      >
        {#if needs_account}
          <div>
            <TosCheckbox bind:termsAccepted />
            <NewsletterCheckbox bind:emailAccepted />
          </div>
        {/if}

        <button
          disabled={(needs_account && !termsAccepted) || loggingIn}
          class="btn btn-secondary w-full gap-2"
          onclick={handleGithubLogin}
        >
          {#if loggingIn}
            <div
              in:fade={{ duration: 150 }}
              class="flex h-6 w-6 items-center justify-center"
            >
              <span class="loading loading-spinner h-4 w-4"></span>
            </div>
          {:else}
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-6 w-6"
              viewBox="0 0 24 24"
            >
              <path
                fill="currentColor"
                d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"
              />
            </svg>
          {/if}

          Sign {needs_account ? 'up' : 'in'} with GitHub
        </button>
      </div>
    </div>
  </div>
</div>
