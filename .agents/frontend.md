...
- no translucent colours anywhere, only solid steps of the neutral scale: no `text-*/NN`, `bg-*/NN`, `border-*/NN`, `ring-*/NN`, `divide-*/NN`, no `opacity-*` to dim text or icons, no `rgb(... / a)` / `rgba()` / `color-mix(..., transparent)` greys in CSS, and never let an icon inherit a translucent colour from its parent. Translucent greys compound where strokes overlap and go muddy over lit backgrounds. Composite the old value over the surface it sits on and take the nearest neutral step. On the page (`surface-root`): text /80 → `text-neutral-300`, /70 and /60 → `text-neutral-400`, /50 and /45 → `text-neutral-500`, /40 and /30 → `text-neutral-600`, /20 → `text-neutral-700`; fills /5 → `bg-neutral-900`, /10 to /15 → `bg-neutral-800`; lines up to /10 → `border-hairline`, /20 → `border-neutral-700`, /30 → `border-neutral-600`. On a card (`surface-elevated`) fills go one step lighter and /40 text is `text-neutral-500`. `npm run lint` in `apps/frontend` runs `scripts/check-solid-colours.mjs`, which fails the build on any of these. The only translucency that stays is see-through by design (modal scrims, glass headers, fade masks to transparent, box shadows, the hero stage lighting), allowlisted in that script.
...
Colours come from semantic tokens in `packages/hyper-ui/src/lib/styles/tokens`.
`primitives.css` holds the raw neutral scale (the `neutral-*` utilities) and `semantic.css` maps it to semantic CSS variables on `:root`.
The app is dark only; a light theme would override the same semantic variables under its own selector.
Every semantic variable is exposed to Tailwind as `--color-<name>` in hyper-ui `main.css`, so `--surface-root` is `bg-surface-root`, `--fg-muted` is `text-fg-muted` and `--border-default` is `border-border-default`.
Scoped component CSS reads the variables directly: `background-color: var(--surface-elevated)`.
`--focus-ring` and `--focus-ring-error` are box-shadow values, used as `shadow-(--focus-ring)` or `box-shadow: var(--focus-ring)`.

| Token                   | Value          | Use                                                     | Replaces (daisyUI)                                               |
| ----------------------- | -------------- | ------------------------------------------------------- | ---------------------------------------------------------------- |
| `surface-root`          | neutral-950    | the page, cut-outs in the page colour                   | `base-300`                                                       |
| `surface-elevated`      | neutral-900    | cards, panels, menus                                    | `base-200`                                                       |
| `surface-100`           | neutral-800    | raised fills: inputs, chips, pressed rows               | `base-100` as a fill, `neutral`, `surface-root-selected`         |
| `surface-150`           | neutral-700    | one step above `surface-100`, its hover                 |                                                                  |
| `surface-well`          | neutral-950    | an inset well inside a card                             |                                                                  |
| `surface-hover`         | neutral-900    | a row hovered on the page                               | `surface-root-hover`                                             |
| `surface-inverse`       | neutral-100    | light fills: primary buttons, dots                      | `secondary` and `base-content` as a fill, the `btn-primary` fill |
| `surface-inverse-hover` | neutral-200    | hover of an inverse fill                                |                                                                  |
| `fg-default`            | neutral-100    | text                                                    | `base-content`, `secondary`, `*-content` on dark fills           |
| `fg-secondary`          | neutral-400    | supporting text                                         |                                                                  |
| `fg-muted`              | neutral-500    | hints, captions                                         |                                                                  |
| `fg-faint`              | neutral-600    | disabled and placeholder text                           |                                                                  |
| `border-subtle`         | #222225        | structural hairlines (same as `hairline`)               |                                                                  |
| `border-default`        | neutral-800    | control and card edges                                  | `base-100` as a line                                             |
| `border-strong`         | neutral-700    | emphasised edges, hover edges                           |                                                                  |
| `hairline`              | #222225        | section dividers, column lines, tile grids              |                                                                  |
| `brand`                 | neutral-100    | accent: focus borders, selected states, emphasised text | `primary` in utilities (`text-primary`, `bg-primary/10`, ...)    |
| `error`                 | red-600        | errors                                                  | `error`                                                          |
| `error-bg`              | red-600 at 15% | danger button fill                                      | `btn-error` fill                                                 |
| `error-bg-hover`        | red-600 at 25% | danger button hover                                     | `btn-error` hover                                                |
| `success`               | green-600      | success                                                 | `success`                                                        |
| `warning`               | amber-500      | warnings                                                | `warning`, `warning-content`                                     |
| `info`                  | blue-600       | information                                             | `info`                                                           |
